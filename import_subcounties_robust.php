<?php
/**
 * Import Subcounties Data Robust
 * This script will import subcounties using a more robust approach
 */

require_once 'utils/dbaccess.php';

echo "Import Subcounties Data Robust\n";
echo "=============================\n";
    ...
    for ($i = 0; $i < count($values); $i += $batchSize) {
        ...
        if (!empty($cleanBatch)) {
            ...
            if ($dbAccess->conn->query($insertSQL)) {
                ...
                foreach ($cleanBatch as $record) {
                    $singleSQL = "INSERT INTO sub_counties (id, uuid, districtCode, countyCode, subCountyCode, subCountyName, deleted_at, created_at, updated_at) VALUES $record";
                    if ($dbAccess->conn->query($singleSQL)) {
                        $totalInserted++;
                    } else {
                        echo "❌ Failed single record: " . $dbAccess->conn->error . "\n";
                        echo "Record: $record\n";
                        break;
                    }
                }
